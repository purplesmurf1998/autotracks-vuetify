<template>
  <div>
    <v-alert :color="messageType" v-if="message" text outlined dense class="mx-auto mt-4 mb-0" style="width: 90%">{{ message }}</v-alert>
    <v-row class="mx-auto" style="width: 90%">
      <v-col class="pl-0 mt-5">
        <p class="text-h5 font-weight-light">Account Roles</p>
        <v-card>
          <v-list class="pa-0">
            <v-list-item-group v-model="selectedRoleIndex" color="primary" mandatory>
              <v-list-item v-for="(role) in roles" :key="role._id" @click="toggleRole(role)">
                <v-list-item-content>
      ...
      <v-col class="pr-0 mt-5">
        ...
        <v-row v-if="werePermissionsModified">
    ...
    <v-dialog v-model="editingRole" max-width="300">
      <v-card>
        ...
        <v-card-text>
          <v-text-field outlined v-model="selectedRole.title" v-if="selectedRole" dense hide-details/>
        </v-card-text>
        <v-divider></v-divider>
        <v-card-actions>
          <v-spacer></v-spacer>
    ...
    <v-dialog transition="dialog-bottom-transition" width="500" v-model="deletingRole">
      <delete-dialog 
...
export default {
  ...
  computed: {
    werePermissionsModified() {
      ...
      this.selectedRole.permissions.forEach(value => {
        ...
        Object.keys(value.policy).forEach(key => {
          if (this.selectedRolePermissions[key + '_' + resource] != value.policy[key]) changes = true;
        })
      })
    ...
    permissions() {
      return Object.keys(this.selectedRolePermissions).filter(title => title.includes(this.permissionSearch));
    }
  },
  methods: {
    deleteRole() {
      axios.delete(`${this.$store.state.baseApiUrl}/roles/${this.selectedRole._id}`).then(() => {
        this.roles.splice(this.selectedRoleIndex, 1)
    ...
    saveRoleTitle() {
      axios.put(`${this.$store.state.baseApiUrl}/roles/${this.selectedRole._id}`, {
    ...
    saveChanges() {
      ...
      this.selectedRole.permissions.forEach((value, index) => {
        ...
        Object.keys(value.policy).forEach(key => {
          temp_permissions[index].policy[key] = this.selectedRolePermissions[key + '_' + resource];
        });
      });
    ...
    fetchRoles() {
      ...
      axios.get(`${this.$store.state.baseApiUrl}/roles`, { 
        params: { 
