  ...
  methods: {
    ...
    save() {
      ...
      for (var filter in this.filters) {
        //Remove empty unused filters when the payload is sent to the inventory component
        if (this.filters[filter].length == 2) {
          if (
            this.filters[filter][0] != null ||
            this.filters[filter][1] != null
          )
      ...
      DealershipService.getProperties(this.activeDealershipId)
        .then((response) => {
          ...
          response.data.forEach((property) => {
            if (
              property.input_type == "Currency" ||
              property.input_type == "Number" ||
              property.input_type == "Date"
            )
              tempFilters[property.key] = [null, null];
            else tempFilters[property.key] = [];
            tempFields.push({
            ...
            if (
              ...
              //This if statement is supposed to set the min and max rules for certain properties
              tempRules[property.key] = (v) => {
                if (!tempFilters[property.key][0] || v == null)
                  //If one of the values or both are not defined, return true and use the value that is defined as a filter
                  return true;
          ...
          this.filters = this.inventoryFilters
    ...
    fetchVehicles() {
      //This method is called when the modal is rendered to fetch all possible values for each vehicle property
        ...
        .then((response) => {
          ...
          response.data.forEach((vehicle) => {
            for (const property in vehicle.properties) {
              ...
              if (
                !propertyValues[property].includes(
                  vehicle.properties[property].value
                ) &&
                vehicle.properties[property].value != null &&
                vehicle.properties[property].value.length != 0
              )
                propertyValues[property].push(
                  vehicle.properties[property].value
                );
            }
          });
