<template>
  <v-card>
    <v-card-title> Edit Vehicle Properties </v-card-title>
    <v-card-text>
      <v-form
    ...
    <v-card-actions>
      <v-spacer></v-spacer>
      <v-btn color="primary" @click="validateVehicleForm"> Save Changes </v-btn>
...
export default {
  ...
  methods: {
    ...
    save() {
      ...
      vehicleObj.properties = this.vehicleProperties;
        ...
        .then((response) => {
          ...
          response.data.forEach((property) => {
            ...
            if (property.input_type == "List") {
              this.$watch(
                `vehicleProperties.${property.key}.value`,
                (val, prev) => {
                  if (!prev) return;
  ...
  mounted() {
    ...
    this.vehicleProperties = JSON.parse(
      JSON.stringify(this.vehicle.properties)
    );
